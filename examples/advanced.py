...
def main():
    instance = Memoryweave(config={
        "timeout": 30,
        "retries": 3,
        "debug": True,
    })

    print("=== Advanced memoryweave Example ===\n")

    # Batch processing
    inputs = [f"data_{i}" for i in range(10)]
    results = [instance.store(input=inp, index=i) for i, inp in enumerate(inputs)]
